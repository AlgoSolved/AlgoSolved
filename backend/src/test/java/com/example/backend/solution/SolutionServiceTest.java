...
public class SolutionServiceTest {
    ...
    class GetSolutionTest {
        ...
        public void solutionIsExistGetTest() {
            Solution solution = Solution.builder().language(faker.programmingLanguage().name())
                    .problem(problem).build();
            SolutionDetailDTO expectedSolutionDto = SolutionDetailDTO.mapToDTO(solution);

            when(solutionRepository.findById(solution.getId())).thenReturn(
                    Optional.of(solution));

            SolutionDetailDTO solutionDetailDto = solutionService.getSolution(solution.getId());
            Assertions.assertEquals(expectedSolutionDto.getProblemName(),
                    solutionDetailDto.getProblemName());
            Assertions.assertEquals(expectedSolutionDto.getLanguage(),
                    solutionDetailDto.getLanguage());
            Assertions.assertEquals(expectedSolutionDto.getSourceCode(),
                    solutionDetailDto.getSourceCode());
        }
